import { Icon } from "@iconify/react";

export const RowTableHistoryMedic = ({ historiaMedica }) => {
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      <td className="px-4 py-3 align-middle text-center">{historiaMedica.id}</td>
      <td className="px-4 py-3 align-middle text-center">{historiaMedica.patient.dni}</td>
      <td className="px-4 py-3 text-center align-middle">
        <span
          className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${
            historiaMedica.status
              ? "bg-emerald-50 text-emerald-700"
              : "bg-rose-50 text-rose-700"
          }`}
        >
          {historiaMedica.status ? "Activa" : "Inactiva"}
        </span>
      </td>
      <td className="px-4 py-3 align-middle text-center">
        {new Date(historiaMedica.createdAt).toLocaleDateString("es-ES", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        })}
      </td>
      <td className="px-4 py-3 align-middle text-center">
        {new Date(historiaMedica.updatedAt).toLocaleDateString("es-ES", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        })}
      </td>
      <td className="px-4 py-3 align-middle text-center  ">
        <div className="inline-flex items-center gap-2">
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-cyan-50 text-cyan-700 hover:bg-cyan-100 transition-colors">
            <Icon icon="tabler:pencil" width="18" height="18" />
          </button>
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-rose-50 text-rose-700 hover:bg-rose-100 transition-colors">
            <Icon icon="gg:trash" width="16" height="16" />
          </button>
        </div>
      </td>
    </tr>
  );
};
